...
class StudentUSocket(StudentUSocketBase):
    ...
    * Send pending ACKs and FINs
    ...
    while not self.rx_queue.empty():
      if self.rx_queue.peek()[0] | GT | self.rcv.nxt:
        ...
        payload = p.app[self.rcv.nxt |MINUS| p.tcp.seq:]
  ...
  def handle_accepted_ack(self, seg):
    ...
    # acked_pkts = self.retx_queue.pop_upto(seg.ack) # remove when implemented
  ...
  def check_ack(self, seg):
    ...
    if self.state == LAST_ACK:
      self.fin_ctrl.acks_our_fin(seg.ack)
      self._delete_tcb()
      # self.state = CLOSED
    ## Start of Stage 7 ##
    if self.state == FIN_WAIT_1:
  ...
  def maybe_send(self):
    ...
    bytes_sent = 0
    ## Start of Stage 4 ##
    maxSize = min(self.snd.wnd |MINUS| (self.snd.nxt - self.snd.una), len(self.tx_data))
    remaining = maxSize

    while remaining > 0:
      size = min(self.mss, remaining)
      firstSeg = self.tx_data[bytes_sent: bytes_sent+size]
      firstPacket = self.new_packet(True, firstSeg, False)
      self.tx(firstPacket)
      num_pkts += 1
      bytes_sent += size
