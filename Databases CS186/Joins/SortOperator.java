...
public class SortOperator {
    ...
    public Run sortRun(Run run) {
        ...
        ArrayList<Record> pages = new ArrayList<>();
        Run newRun = createRun();
        while (runIterator.hasNext()) {
            pages.add(runIterator.next());
        }
        pages.sort(comparator);
        newRun.addRecords(pages);
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        PriorityQueue<Record> record = new PriorityQueue<>(comparator);
        for (Run run : runs) {
            Iterator<Record> runIterator = run.iterator();
            while (runIterator.hasNext()) {
                record.add(runIterator.next());
            }
        }
        Run newRun = createRun();
        while (!record.isEmpty()) {
            newRun.addRecord(record.poll().getValues());
        }
        return newRun;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int runsIndex = 0;
        ArrayList<Run> ret = new ArrayList<>();
        int noRuns = 1;

        for (int i = 0; i < Math.floorDiv(runs.size(), numBuffers - 1); i++) {
            ArrayList<Run> bruns = new ArrayList<>();
            for (int inner = 0; inner < numBuffers - 1; inner++) {
                bruns.add(runs.get(runsIndex));
                runsIndex += 1;
            }
            Run merged = mergeSortedRuns(bruns);
            ret.add(merged);
            noRuns += 1;
        }
        for (int last = numBuffers * noRuns; last < runs.size(); last++) {
            ArrayList<Run> bruns = new ArrayList<>();
            bruns.add(runs.get(last));
            Run merged = mergeSortedRuns(bruns);
            ret.add(merged);
        }
        return ret;
...
//        int runsIndex = 0;
//        int size = runs.size();
//        while (runsIndex < size) {
//            if (index < numBuffers -1) {
//                bruns.add(index, runs.get(runsIndex));
//                runsIndex += 1;
//                index += 1;
//            } else {
//                Run merged = mergeSortedRuns(bruns);
